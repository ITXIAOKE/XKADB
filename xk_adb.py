...
from adb.app_adb_time import collect_time
from adb.app_adb_monkey import collect_monkey
from adb.app_adb_state import collect_state
from adb.app_adb_cpu import collect_cpu
from adb.app_adb_flow import collect_flow
from adb.app_adb_mem import collect_mem
from utils.operate_execl_util import qidongceshi, getcpu
from utils.operate_log_util import LOG, logger


@logger('启动app时间测试')
def start_app_time_func():
    start_app_time = []
    cishu = []
    # 设备状态
    status_shebei = collect_state()
        ...
        if len(acti) <= 1 or len(packname) <= 1:
            ...
            if len(cish) <= 1:
                ...
                for i in range(int(cish)):
                    start_time = collect_time(packagename=packname, packagenameactivicy=acti)
                    start_app_time.append(int(start_time[1]))
                ...
                qidongceshi(cishu=cishu, start=start_app_time)
...
def monkey_app():
    ...
    if status_shebei == 'device':
        try:
            ...
            collect_monkey(packagename=packname, s_num=zhongzi, throttle=time, pct_touch=touch, pct_motion=huadong,
...
def app_performance():
    ...
    if status_shebei == 'device':
        ...
        for i in range(int(xing)):
            ...
            # # 每次延时一秒统计数据
            # time.sleep(1)
            nen_cun = collect_mem(xingneng_bao)
            rescv, send, liulang_sum = collect_flow(xingneng_bao)
            cpu_caiji = collect_cpu(xingneng_bao)
            neicun_t['state'] = 'normal'
            pass_list.append(int(nen_cun[:-1]))
            neicun_t.insert(tkinter.END, ('内存值：%s' % nen_cun))
            LOG.info('第%s次：内存值：%s' % (i, nen_cun))
        ...
        getcpu(cishu=cishu_list, start_cpu=cpu_list, recv_list=rescv_list, send_list=send_list, total_list=total_list,
               pass_list=pass_list)
...
@logger('调用线程来启动测试！采集cpu占用率,上传下载流量，内存')
def start_app_performance_thread():  # 如果不是ui界面，可以不用线程
    for i in range(1):
        t = threading.Thread(target=app_performance, args=())
        t.start()


@logger('启动app时间线程测试')
def start_app_time_thread():
    for i in range(1):
        t = threading.Thread(target=start_app_time_func, args=())
        t.start()


if __name__ == '__main__':
    LOG.info('测试程序开始启动！测试开启！')
    try:
        status_shebei = collect_state()
        if status_shebei == 'device':
            # GUI图形化库初始化
            root = tkinter.Tk()
            # 设置标题
            root.title('小可的安卓adb monkey性能测试小工具')
            # 设置窗口大小
            # root.geometry("900x300")
            # 设置窗口是否可以变换长宽，False不可以变换
            ...
            btn_start = tkinter.Button(root, text='开启时间测试', font=("黑体", 15, "bold"), command=start_app_time_thread,
                                       compound='center', bg='red')
            ...
            log_t.insert('0.0', 'E:\\pycharmproject\\adbproject\\xh_adb\\log\\monkey.txt')

            btn_monkey = tkinter.Button(root, text='开启Monkey测试', font=("黑体", 15, "bold"), command=monkey_app,
                                        compound='center', bg='red')
            btn_monkey.grid(row=17, column=3)
            print("=============================================================================================")
            # mainloop就进入到事件（消息）循环。一旦检测到事件，就刷新组件。
            root.mainloop()
        else:
